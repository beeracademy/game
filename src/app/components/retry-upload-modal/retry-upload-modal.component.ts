...
export class RetryUploadModalComponent implements OnInit {
  ...
  public dismiss() {
    this.modal
      .openConfirm(
        "If you dismiss this dialog the game will not be uploaded. Want to dismiss?"
      )
      .subscribe((result) => {
        if (result) {
  ...
  public download() {
    const dataStr =
      ...
      encodeURIComponent(JSON.stringify(this.data.game));
    const downloadAnchorNode = document.createElement("a");
    downloadAnchorNode.setAttribute("href", dataStr);
    downloadAnchorNode.setAttribute(
      "download",
      "game_" + this.data.game.id + ".json"
    );
  ...
  private upload() {
    return this.http
      .post(
        `${environment.url}/api/games/` + this.data.game.id + "/update_state/",
        this.data.game
      )
      .subscribe(() => {
        clearInterval(this.intervalRef);
